  ...
  - {formatted_languages}"""
    ...
    async def get_stats(self) -> None:
        ...
        while True:
            ...
            for repo in tqdm(repos, desc="Processing Repos (GraphQL Data)"):
                if repo is None:
                    print("Found None repo in repos list")
                    continue
    ...
    async def lines_changed(self) -> Tuple[int, int]:
        ...
        additions = 0
        deletions = 0
        # Iterate over repos with progress bar
        # Note: await self.repos returns a list, so we can wrap it
        repo_list = await self.repos
        for repo in tqdm(repo_list, desc="Fetching Contributor Stats"):
    ...
    async def views(self) -> int:
        ...
        total = 0
        repo_list = await self.repos
        for repo in tqdm(repo_list, desc="Fetching Traffic Stats"):
